...
def init_system(self):
    ...
    if self.particle in ['raspberry', 'polymer']:
...
def configure_system(self):
    ...
    self.s.min_global_cut = 1.1 * self.param._max_bond_ext
...
def add_fene_and_harm(self):
    ...
    # interactions
    # fene = FeneBond(k=self.param._Kbond, d_r_max=self.param._max_bond_ext)
    fene = HarmonicBond(k=self.param._Kbond*10000, r_0=self.param._lj_sigma)
    hb = HarmonicBond(k=self.param._Kbond, r_0=self.param._lj_sigma)
    # fene_mnps = FeneBond(k=self.param._Kbond*2, d_r_max=self.param._max_bond_ext-.95*self.param._sigma_m) #-0.5*self.param._sigma_m
    fene_mnps = FeneBond(k=self.param._Kbond*2.25, d_r_max=.95*self.param._sigma_m)
    # hb_tmp = HarmonicBond(k=self.param._Kbond*1, r_0=self.param._lj_sigma*1.2) #for equilibrating frozen mnps
    hb_tmp = HarmonicBond(k=self.param._Kbond*100, r_0=self.param._lj_sigma) #for equilibrating frozen mnps
    # hb_tmp = HarmonicBond(k=self.param._Kbond*2.25*100, r_0=self.param._lj_sigma*0.5) #for equilibrating frozen mnps
...
def replace_hb_tmp_with_fene(self):
    """
    ...
    for p in self.s.part.all():
        if len(p.bonds) == 1:
            connected_to = p.bonds[0][1]
            p.delete_bond(p.bonds[0])
            p.add_bond((0,connected_to))

def replace_fene_with_hb(self):
    """
    hb_tmp used for polymers while warm up, which supposed to be changed to fene for prod. simulations

    """
    self.s.min_global_cut = 2 * self.param._max_bond_ext
    self.s.cell_system.skin = 2 * self.param.skin

    hrm = HarmonicBond(k=self.param._Kbond*10000, r_0=0.2)
    self.s.bonded_inter.add(hrm)
    # fene_mnps = FeneBond(k=self.param._Kbond, d_r_max=self.param._max_bond_ext)
    # self.s.bonded_inter.add(fene_mnps)
    ...
    for p in self.s.part.all():
        if len(p.bonds) == 1:
            connected_to = p.bonds[0][1]
            p.delete_bond(p.bonds[0])
            p.add_bond((hrm,connected_to))
            # p.add_bond((fene_mnps,connected_to))

            # pos, pos1 = p.pos, self.s.part.by_id(connected_to).pos
            # dist = np.linalg.norm(pos-pos1)
            # if dist >= 2:
            #     print('dist between particles is too large', dist, p.id, connected_to)
...
def add_walls_and_shear_flow(self, shear_flow=False):
    ...
    wall_top = shapes.Wall(normal=[0, 0, -1.0], dist=-(self.param._box_l - 1*self.param._gap))
    ...
    if shear_flow == True:
        ...
        shear_profile = np.linspace(0, v, int(self.param._box_l - 2*self.param._gap))

        for idx, sh in enumerate(shear_profile[:]):
            # optimize code below for speed with numpy
            ...
            self.lbf[:, :, idx+int(self.param._gap)].velocity = np.full((int(self.box_len[0]), int(self.box_len[1]), 1, 3), np.array([sh, 0, 0]))
...
def impulse_y_pos_shear(self):
    self.s.lbboundaries[1].velocity = (self.param.shear_rate*self.param._box_l, self.param.shear_rate*self.param._box_l, 0)
    self.s.lbboundaries[0].velocity = (0, -self.param.shear_rate*self.param._box_l, 0)

def impulse_y_neg_shear(self):
    self.s.lbboundaries[1].velocity = (self.param.shear_rate*self.param._box_l, -self.param.shear_rate*self.param._box_l, 0)

def impulse_y_stop_shear(self):
    self.s.lbboundaries[1].velocity = (self.param.shear_rate*self.param._box_l, 0, 0)
    self.s.lbboundaries[0].velocity = (0, 0, 0)
...
def gen_observ_forces(self):
    ...
    elif self.particle == 'polymer':
        ids = []
        for p in self.s.part.all():
            ids.append(p.id)
        part_force = ParticleForces(ids=ids)
        self.part_force = part_force
...
def run(self, timesteps):
    try:
        ...
        # try:
        if "Harmonic bond: Particles have zero distance." in str(sys.exc_info()):
            pass
        else:
            ...
            p = self.s.part.by_id(pid1)
            p.delete_bond(p.bonds[0])

            with open(self.project_path + "/brocken_bonds", "a") as file:
                file.write(str(self.s.time)+ " "+ str(pid) + " "+ str(pid1)+ "\n")

        t_steps_done = int((self.s.time - t_begin)/self.s.time_step)

        print("pos_dip_id=", self.pos_dip_id, "timesteps=", timesteps-t_steps_done)

        self.run(timesteps-t_steps_done)
        # except:
        #     self.process_sim_except()
...
def shift_particle_to_center(self):
    """
    ...
    if self.z_coord:
        ...
        pos_z = (self.box_len[2]+2*self.param._gap)/2
...
def update_top_wall_vel(self):
    """
    for oscillating shear
    """

    if self.pos_dip_id == 0:
        def sinusoidal_signal(t, A, omega, phi):
            return A*np.sin(omega*t + phi)

        t = np.linspace(0, self.num_of_steps, self.num_of_steps)

        A = self.param.shear_rate*self.param._box_l*self.param.shear_amp
        self.param.signal = sinusoidal_signal(t, A, 2*np.pi*1/self.shear_freq, 0)

    # if self.pos_dip_id // 16:
    self.s.lbboundaries[1].velocity = (self.param.signal[self.pos_dip_id], 0, 0)
    self.s.integrator.run(0)
