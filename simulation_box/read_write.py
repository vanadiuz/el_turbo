...
def write_pos_dip_hdf5(self):
    ...
    ds_dtype = np.dtype([
        ('step', 'int'),
        ('pos_dip',ps_dp, len(self.s.part) if len(self.s.part) > 0 else 1)
        ])
    
    #get empty array with ds_dtype
    if len(self.s.part) > 0:
        ds_arr = np.rec.array(self.pos_and_dips_hdf5, dtype=ds_dtype)
    else:
        ds_arr = np.rec.array(np.rec.array(np.zeros(1, dtype=ds_dtype)), dtype=ds_dtype)
...
def load_pos_of_equlibrated_spheres(self, path, num_of_spheres):
    """
    Read pos from .h5 file
    """
    
    path += "/N_part_"+str(num_of_spheres)+"/pos_dip.h5"
...
def read_ts_mult_history(self):
    ...
    Write ts_mult history to file
    """
    # /gpfs/data/fs70806/Vania/all_in_flow/mng/test_nopart/
    #read ts_mult_history.csv into pandas df where the column names are the first row: step_id, ts_mult
    self.ts_mult_history = pd.read_csv(self.project_path.replace('_no_part', '').replace('_noise', '') + "/ts_mult_history.csv", index_col=None, header=None)
    #get index of the last duplicate of the first column
    

    last_duplicates_index = self.ts_mult_history[self.ts_mult_history.duplicated(keep='first')].index
    values = self.ts_mult_history.loc[last_duplicates_index, 0].tolist()
    self.restart_at = [v for v in values if v % 10 == 0]

    #get index of the last duplicate of the first column
    self.ts_mult_history.drop_duplicates(subset=0, keep='last', inplace=True)
    self.ts_mult_history.reset_index(drop=True, inplace=True)
