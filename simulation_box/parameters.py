...
class Parameters: 
    ...
    #system parameters
    periodicity             : List[float] = field(default_factory=list)
    skin                    : float       = 1
    ...
    def __post_init__(self): 
        ...
        ## 🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱
        ## 🪱🪱🪱🪱 PAAm_polymer (single bead properties)🪱🪱🪱🪱
        ## 🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱🪱

        if self.particle == 'polymer':
            
            dens_gel = 1.0*dens_w

            ### Arbitrary properties (SI units)
            sigma_gel = 4.e-9  # total diameter, m
            R_h = sigma_gel/2. # hydrodynamic radius, m

            # derived properties (SI units)
            mass_gel = dens_gel * pi * (sigma_gel**3.) / 6.  # mass, kg
            momI_gel = mass_gel * (sigma_gel**2.) / 10.      # moment of inertia, kg m^2

            gamma_t_gel = 6.*pi*eta_w*R_h  # translational friction coefficient, kg/s
            gamma_r_gel = 8.*pi*eta_w*((R_h)**3.)  #
        ...
        scale_on_mass_of = mass_gel if self.particle in ['mng', 'raspberry', 'polymer'] else mass_m
        ...
        #!!!!!!!!!!!!
        self._gamma_t_m = gamma_t_gel*self.t_/self.mass_
        self._sigma_m   = sigma_gel/self.d_
        self._mass_m    = mass_gel/self.mass_
        self._momI_m    = momI_gel/(self.mass_*self.d_*self.d_)
        self._mu        = dip/dip_
        self._gamma_r_m = gamma_r_gel*self.t_/(self.mass_*self.d_*self.d_)
        ...
        R_h_factor = 1 if self.particle in ['mng', 'cluster', 'polymer'] else 0.5 #1
...
def read_param_from_args(self):
    ...
    parser.add_argument("--shear_freq", action="store", dest="shear_freq", type=float)
    parser.add_argument("--shear_amp", action="store", dest="shear_amp", type=float)
    ...
    parser.add_argument("--p_length", action="store", dest="p_length", type=int)
    # parser.add_argument("--no_dipdip", action="store", dest="no_dipdip", type=int)
    ...
    if self.args["shear_freq"] != None:
        self.shear_freq = self.args["shear_freq"]
    else:
        warnings.warn("shear_freq wasn't given")

    if self.args["shear_amp"] != None:
        self.shear_amp = self.args["shear_amp"]
    else:
        warnings.warn("shear_amp wasn't given")
    ...
    if self.args["p_length"] != None:
        self.p_length = int(self.args["p_length"])
    else:
        warnings.warn("p_length wasn't given")
        self.p_length = None
    ...
    # # # #GEL BUMP
    self.project_name = "test_pl_50_box157"
    self.script_folder = "./"
    self.project_path = "./data/"
    self.pypresso=''
    self.timesteps = 1550# 155000
    self.particle = 'polymer'
    self.conf = ''#'1_0'#0
    self.lmbda = 5
    self.hmf = 0
    self.hmf_direct = 'x'
    self.hmf_freq = 0#.5*2895408
    self.N_part = 57#30#5730#5730#573#0#570
    self.p_length = 50
    self.box_len = 157#216#216 #216
    self.args["shear_rate"] = 1.51e10#1*1.51e10 #*1.51
    self.new_continue_sim = "new"
    self.num_of_steps = 200000
    self.corr_tau_max = 0#10000
    self.extra_mnps = 0
    self.thermostat = 'lb' #or 'lb'
    self.no_dipdip = 0
    self.fixed_mnps = 0
    self.timestep_size = 0.0001#*4
    self.tmstep_size_lb = 0.0001#*16
    self._m_scale = 1.6
    self._t_scale = 0.0001#*4 #setted in system
    self.part_radius = 7
    self.zero_coupling = False
    self.add_init_vel_z = 0
    self.g = 0#3.6e-12
    ...
    # self._e_scale = 3.875e-7
    # self._v_factor = 1e-05
    ...
    self.prefix = "taskset -c 64 mpirun -np 8 "
    # self.prefix = ""
