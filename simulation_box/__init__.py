...
class SimulationBox(object):
    ...
    def load_system_from_checkpoint(self):
        ...
        if self.thermostat == 'lb':
            ...
            # lbf.tau = 0.0000001
            self.lbf = lbf
            print('lbf.tau = ', self.lbf.tau)


            # print('lbf.tau', self.lbf.tau)

            # self.lbf.kT = self.param._kT
            # print('lbf.kT', self.lbf.kT)

        # param._timesteps = 1550
        self.param = param
    ...
    def init_parameters(self):
        ...
        if self.args["shear_amp"]:
            self.param.shear_amp = self.args["shear_amp"]

        if self.args["p_length"]:
            self.param._p_length = self.args["p_length"]
        elif self.p_length:
            self.param._p_length = self.p_length

        if self.args["shear_freq"]:
            self.param.shear_freq = self.args["shear_freq"]
    ...
    def write_sim_data(self, step, checkpoint_step=10):
        ...
        if self.thermostat == 'lb' and len(self.s.lbboundaries) > 0:
    ...
    def write_sim_data_test(self, step, checkpoint_step=10):
        # self.energies.append(self.get_energy())
        # self.vtfs.append(self.get_vcf(types=[0,1]))
        # self.force_onto_wall.append(self.get_force_onto_wall())
        if step % checkpoint_step == 0:
            # self.write_energy()
            # self.write_force_onto_wall()
    ...
    def squize(self, radius, center, shift_particle_to_center=True):
        ...
        if self.param._hmf == 0:
            ...
            for step in range(1600):
                ...
                try:
                    self.run(int(self.param._timesteps/10))
                except:
                    print("Brock")
    ...
    def process_sim_except(self, new_ts_mult = None):
        
        # print("Brocken bonds")
        # if new_ts_mult is None:
        #     self.tmp_param.ts_mult -= 1 #decrease simulation speed
        # else:
        #     self.tmp_param.ts_mult = new_ts_mult
    ...
    def add_mngs(self, mng, equalibrated = False):
    ...
    def add_polymers(self, polymer, periodicity=3):
        polymer.bake(self, self.param._p_length, self.param._N_part, periodicity)
        polymer.initialize(self, self.new_continue_sim)
    ...
    def add_raspberies(self, rasp, r = 3):
        ...
        if self.N_part != 1:
            ...
            positions = [[int(self.box_len[0]/2),int(self.box_len[1]/2),self.z_coord]]
    ...
    def generate_list_of_configs_to_load(self):
    ...
    def cross_link_polymers(self):
        num_of_crosslinkers = int(0.1*self.N_part*self.p_length)
        grid_size = 5
        #create grid 
        num_of_x_sites = int(self.box_len[0]/grid_size)
        num_of_y_sites = int(self.box_len[1]/grid_size)
        num_of_z_sites = int((self.box_len[2]-2*self.param._gap)/grid_size)

        #create list of len num_of_x_sites with nested lists of len num_of_y_sites with nested lists of len num_of_z_sites
        grid = [[[[] for z in range(num_of_z_sites)] for y in range(num_of_y_sites)] for x in range(num_of_x_sites)]

        #create list of polymers
        for p in self.s.part.all():
            if p.type == 0:
                grid[int(p.pos[0]%self.box_len[0]//grid_size)][int(p.pos[1]%self.box_len[0]//grid_size)][int((p.pos[2]-self.param._gap)//grid_size)].append(p)

        #cross link polymers
        while num_of_crosslinkers != 0:
            x = random.randint(0, num_of_x_sites-1)
            y = random.randint(0, num_of_y_sites-1)
            z = random.randint(0, num_of_z_sites-1)

            if len(grid[x][y][z]) > 1:
                p1 = random.choice(grid[x][y][z])
                p2 = random.choice(grid[x][y][z])

                if p1 != p2 and p1.id%self.p_length != p2.id%self.p_length:
                    try:
                        p1.add_bond((1, p2))
                        num_of_crosslinkers -= 1
                    except:
                        continue
    ...
    from .system import init_system, configure_system, add_fene_and_harm, \
        ...
        update_top_wall_vel, replace_hb_tmp_with_fene, replace_fene_with_hb, impulse_y_pos_shear, \
        impulse_y_neg_shear, impulse_y_stop_shear